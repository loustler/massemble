package io.loustler.massemble.sort
...
object MergeSort {
  ...
  def sort[A](list: List[A])(implicit Ord: Ordering[A]): List[A] = {
    ...
    def sort_(pivot: (List[A], List[A])): List[A] =
      pivot match {
        case (Nil, Nil)          => Nil
    ...
    def halfify(x: List[A]): (List[A], List[A]) = {
      ...
      def loop(y: List[A], fs: List[A], ss: List[A]): (List[A], List[A]) =
        y match {
    ...
    def merge(x: List[A], y: List[A]): List[A] = {
      def loop(a: List[A], b: List[A], r: List[A]): List[A] =
        (a, b) match {
